...
\begin{figure}[t]
\centering
\includegraphics[width=0.86\textwidth]{figures/per_prompt_differences.png}
...
\paragraph{Public replay bundle.} \texttt{results/public/bluedot-v0.1/} holds 22 files: the analysis
of record, the resolution manifest, the aggregated and pair-level score tables, all 512 sealed
forecasts, their commitments, all 512 reveals with their salts, the frozen calibration decision with
its observations and evidence, the direction family, transform and predictor fit provenance, a
bundle manifest, and \texttt{CHECKSUMS.sha256} in the format \texttt{sha256sum -c} accepts. It
carries no images: every figure in this paper is regenerated from it. It replays on its own:
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.82\textwidth]{figures/study_overview.png}
\caption{The whole study on one page: the calibration diagnostics that chose the stimulus before
any forecast existed (top), and the final test (bottom).}
\label{fig:overview}
\end{figure}
